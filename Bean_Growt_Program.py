...
def contar_plantas():
    ...
    label_texto_clasificacion2 = tk.Label(ventana, text="Cargando... este proceso puede tardar algunos segundos.", bg='#67b93e', fg='white', font=("Arial Black", 20))
    label_texto_clasificacion2.pack(pady=20)

def process_image_pieces(image_pieces):
    ...
    for i, (position, selected_image) in enumerate(image_pieces.items()):
        print(f"Seleccionando imagen recortada en posición {position} para la predicción.")

        # Guardar la imagen temporalmente
        image_path = f"temp_image_{i + 1}.jpg"
        selected_image.save(image_path)

        # Llamar a la API de Roboflow para obtener las predicciones
        result = cliente_seleccionado.infer(image_path, model_id=modelo_seleccionado)
        ...
        if not predicciones:
            print(f"No se pudo obtener predicciones para la imagen {i + 1}")
            continue

        # Filtrar predicciones con certeza del 45% o más
        filtered_predictions = [pred for pred in predicciones if pred['confidence'] >= 0.55]

        # Convertir la imagen a un arreglo de NumPy para OpenCV
        selected_image_np = cv2.cvtColor(np.array(selected_image), cv2.COLOR_RGB2BGR)
        ...
        # Dibujar cuadros en la imagen para las predicciones válidas
        for prediction in filtered_predictions:
            x = int(prediction['x'])
            y = int(prediction['y'])
            width = int(prediction['width'])
            ...
            # Dibujar el cuadro de predicción en la imagen
            cv2.rectangle(selected_image_np, (int(x - width / 2), int(y - height / 2)),
                          (int(x + width / 2), int(y + height / 2)), color, 4)
            cv2.putText(selected_image_np, f"{prediction['class']} {prediction['confidence']:.2f}",
                        (int(x - width / 2), int(y - height / 2) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

        # Guardar la imagen procesada (con predicciones) en el diccionario
        processed_pieces[position] = selected_image_np
...
# Función para dividir la imagen en subimágenes
def split_image_2(image, size=1280):
    img_width, img_height = image.size
...
def modelo_roboflow():
    ...
    try:
        ...
        # Procesar las subimágenes para obtener predicciones y dibujar cuadros
        processed_image_pieces = process_image_pieces(image_pieces)

          # Reconstruir la imagen final con predicciones
        reconstructed_image = reconstruct_image_with_predictions(processed_image_pieces, num_tiles_x, num_tiles_y)
        ...
        boton_identificar = Button(ventana, text="Finalizar", command=finalizar, font=("Arial", 12), bg="white", fg="black")
        boton_identificar.pack(pady=10)
